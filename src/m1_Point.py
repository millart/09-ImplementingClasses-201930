...
class Point(object):
    ...
    def move_to(self, x, y):
        x_distance = (x - self.x) ** 2
        y_distance = (y - self.y) ** 2
        add_distance = (x_distance + y_distance) ** (1/2)
        self.total_distance = self.total_distance + add_distance
    ...
    def get_distance_from(self, pt):
        x_dir = (pt.x - self.x) ** 2
        y_dir = (pt.y - self.y) ** 2
        distance = (x_dir + y_dir) ** (1/2)
        return distance

    def get_distance_from_start(self):
        x_dir = (self.x - self.start_x) ** 2
        y_dir = (self.y - self.start_y) ** 2
        distance = (x_dir + y_dir) ** (1 / 2)
        return distance
    ...
    def closer_to(self, p2, p3):
        distance_p2 = (((self.x - p2.x) ** 2) + ((self.y - p2.y) ** 2)) ** (1/2)
        distance_p3 = (((self.x - p3.x) ** 2) + ((self.y - p3.y) ** 2)) ** (1/2)
